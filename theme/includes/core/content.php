...
class Content {
	 ...
	 * @return string
	 */
	public function heading($before = '', $after = '', $add_page_number = false) {
	 ...
	 * @return string
	 */
	public function title($object, $length = 0) {
	 ...
	 * @return bool|string
	 */
	public function text($object = false, $length = 250, $allowed_tags = false, $find = ' ', $force_cut = true) {
		...
		if (empty($length)) {
			...
			if ($excerpt and mb_strlen($excerpt) > 10) {
				...
				if ($force_cut) {
					$result = $this->strip($excerpt, $length, $allowed_tags, $find);
				} else {
					$result = $excerpt;
				}
				...
				if ($force_cut) {
					$result = $this->strip(mb_substr($text, 0, $pos), $length, $allowed_tags, $find);
				} else {
					$result = $this->strip(mb_substr($text, 0, $pos), $pos, $allowed_tags, $find);
				}

			} else {

				$result = $this->strip($text, $length, $allowed_tags, $find);
	 ...
	 * @return string
	 */
	public function strip($text, $length = 200, $allowed_tags = false, $find = ' ', $dots = '...') {
	 ...
	 * @return string
	 */
	public function date($post = false, $format = '') {
