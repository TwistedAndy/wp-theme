...
class AcfTest extends WP_UnitTestCase {
	...
	public static function set_up_before_class(): void
	{
		...
		// Mock Twee meta helpers
		if (!function_exists('tw_meta_update')) {
			function tw_meta_update($type, $id, $key, $value)
			{
				return update_metadata($type, $id, $key, $value);
			}
		}

		if (!function_exists('tw_meta_get')) {
			function tw_meta_get($type, $id, $key)
			{
		...
		if (!function_exists('acf_get_field')) {
			function acf_get_field($key)
			{
				...
				// Fallback for previous tests relying on this specific hardcoded value
				if ($key === 'field_grp') {
					return ['type' => 'group'];
				}
	...
	public function test_revision_fields(): void
	{
		...
		// Repeater Check
		$key_rep = 'twee_acf_rep_rep_0_val';
		$this->assertEquals('Rep Item', apply_filters("_wp_post_revision_field_$key_rep", '', $key_rep, get_post($post_id)));
	}
	...
	{
		$post_id = self::factory()->post->create();
		$rev_id = wp_insert_post(['post_type' => 'revision', 'post_parent' => $post_id]);

		$map = ['restored_field' => 'key_123'];
		update_metadata('post', $rev_id, '_acf_map', $map);
		update_metadata('post', $rev_id, 'restored_field', 'Restored Value');

		tw_acf_revision_restore($post_id, $rev_id);
		...
		update_metadata('post', $rev_id, '_acf_map', []); // Empty map
		tw_acf_revision_restore($post_id, $rev_id);
	...
	public function test_revision_create(): void
	{
		...
		$rev_id = wp_insert_post(['post_type' => 'revision', 'post_parent' => $post_id]);
		tw_acf_revision_create($rev_id);

		$this->assertEquals($map, get_metadata('post', $rev_id, '_acf_map', true));
		$this->assertEquals('Parent Value', get_metadata('post', $rev_id, 'field_copy', true));

		// Check Failures
		$not_rev_id = self::factory()->post->create();
		tw_acf_revision_create($not_rev_id);
		$this->assertEmpty(get_metadata('post', $not_rev_id, '_acf_map', true));
	}
